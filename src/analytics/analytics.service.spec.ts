...
describe('AnalyticsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AnalyticsService,
        { provide: getRepositoryToken(MeterTelemetry), useValue: mockMeterRepo },
        { provide: getRepositoryToken(VehicleTelemetry), useValue: mockVehicleRepo },
  ...
  it('should return insufficient data if telemetry is missing', async () => {
    mockMappingRepo.findOne.mockResolvedValue({ vehicleId: 'V1', meterId: 'M1' });
  ...
  it('should calculate efficiency correctly when data is present', async () => {
    ...
    // Start of 24h
    mockVehicleRepo.findOne.mockResolvedValueOnce({ kwhDeliveredDc: 100, timestamp: new Date() });
    // End of 24h
    mockVehicleRepo.findOne.mockResolvedValueOnce({ kwhDeliveredDc: 110, timestamp: new Date(), soc: 80, batteryTemp: 35 });
    
    // Start of 24h
    mockMeterRepo.findOne.mockResolvedValueOnce({ kwhConsumedAc: 200, timestamp: new Date() });
    // End of 24h
    mockMeterRepo.findOne.mockResolvedValueOnce({ kwhConsumedAc: 212.5, timestamp: new Date(), voltage: 230 });
